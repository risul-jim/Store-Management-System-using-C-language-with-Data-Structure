...
FILE *fpp;
   ...
   printf("\n\n\n\t\t\t||----------------------------------------------------||\n\t\t\t|| total income from selling products have been: %i\n\t\t\t||----------------------------------------------------||\n", cn.wholesum);
		...
		printf("\n\tPress 'Enter' to add more item and any other key to go to menu");
getch();
          }
mainT();
...
struct delers* insert_last(char code[],char name[],char phone_num[],char pro_name[]) // Declaration of variables which would be used in the execution of this section
{
    struct delers *temp=delear_head,* new_node=(struct delers*)malloc(sizeof(struct delers)); //declaration of nodes and memory  allocation
    strcpy(new_node->deler_code,code);
    strcpy(new_node->deler_name,name);
    strcpy(new_node->deler_phone_num,phone_num);
    strcpy(new_node->de_product_name,pro_name);   // Value Assignation to the variables using strcpy operator
    new_node->next=NULL;
...
struct delers* arrange_delers(char str[],int num)
{
    ...
    while(temp!=NULL)
    {
        if (!strcmp(temp->deler_code,str)) // Comparing the data assigned in temp  the deler_code by using strcmp() operator
        {
        ...
        temp=temp->next; // Assigning temp to next node so that it could be traversed
    }
    temp=arrange_head;
    if (temp==NULL)
    {
...
void generate_()
{
    ...
    insert_last("27sd","Mr.abdul","014563","Socks      ");    //Pre-inserted data for the dealers informations
...
void add_deler_inf()
{
    ...
    for ( int i=0; i<n; i++) // Loop for how many dealer's info we want to input
    {
        ...
        int num;
        scanf("%d",&num);
        arrange_delers(str,num); // Assigning the inputted values to the variables
    }
    print_dot();
    printf("\n\n               Added Dealer Successfully!\n\n\n");
  while((ap.c = getch()) =='\r');
	mainx();
}
void star_pointer()
...
printf("\n\n\n\t\t\t\t\t |----------------------------------------------------------|\n");
    ...
    int maxx=0;
    struct delers *tem=arrange_head;
    if( tem== NULL) // If the value of pointer "tem" doesn't match to any inserted values
    {
        printf(" \n              NO dealer arranged!      \n\n");
        getch();
        return;
    ...
    while(tem!=NULL)
    {
        if ( tem->deal_pro>maxx)
            new_node=tem,maxx=tem->deal_pro; // The loop will execute the proccess exactly equal to the value of deal_pro
        tem=tem->next;
    }
...
void delate_deler(struct delers** head_ref, char str[])
{
    ...
    if (temp != NULL && (!strcmp(temp->deler_code,str))) // Loop to operate the deletion of dealer's information
    {
        *head_ref = temp->next;
    ...
    while (temp != NULL && (strcmp(temp->deler_code,str))) // This Loop will execute if and only if the dealer's code doesn't match
    {
        prev = temp;
        temp = temp->next;
    }
       if (temp==NULL)
        ...
        {
            printf("\n\n           Delete dealer information successfully!\n");
        }
    prev->next = temp->next;
}
void delate_inf()
...
printf("\n\n\n\n\n\t\t\t\t\t |----------------------------------------------------------|\n");
     ...
     printf("\n\n\n\t\t\t\t\t |----------------------------------------------------------|\n");
          ...
          printf("\t\t\t\t\t |----------------------------------------------------------|\n\n");
    node *Ti,*prev,*Tinv,*Es,*Eb,*Sr; // creating nodes for the variables

    Ti=(node*)malloc(sizeof(node)); // Allocating memory
    ...
    if((fpp=fopen("NextsumFile.dat","r+"))==NULL)
	{
		...
		mainT();
          ...
          else                                   // Inserting the pre-stored data by calling the File
	{
  ...
  fclose(fpp);
    ...
    Ti->data=cn.wholesum; // Assigning value to the "Ti" variable by calling the file
    Ti->next=NULL;
    prev=Ti;
    ...
    Eb=(node*)malloc(sizeof(node));                // Allocating data to the variables
    ...
    double Mcal = Ti->data -(Tinv->data + Eb->data + Es->data + Sr->data); // Final Calculation of the section
